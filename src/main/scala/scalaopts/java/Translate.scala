...
object Translate {
  ...
  def asTypedCommandLineOption[T](opt: ICommandLineOption[T]): TypedCommandLineOption[T, Seq[T], List[T]] = {
    ...
    new FinalTypedCommandLineOption(opt.getName(), opt.isRequired(), long_names, short_names, dependencies, opt.getDescription(), opt.getArity(), opt.getMinNumberOfRequiredValues(), opt.getMaxNumberOfRequiredValues(), None, option_parser, new ListOptionArgumentAccumulator[T])
  }
